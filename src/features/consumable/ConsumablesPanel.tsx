...
const ConsumableItem = ({
  ...
  const getIcon = () => {
    ...
    if (
      item.consumable?.description.includes('Força') ||
...
export function ConsumablesPanel() {
  ...
  const useConsumable = async (consumableId: string) => {
    ...
    try {
      await ConsumableService.consumeItem(gameState.player.id, consumableId, gameState.player);
      ...
      <SheetContent side="right">
        <SheetHeader>
          <SheetTitle className="flex items-center gap-2">
            Consumíveis
            {potionUsedThisTurn && (
              <span className="text-orange-400 text-sm">• Poção usada neste turno</span>
            )}
          </SheetTitle>
        </SheetHeader>
        <div className="mt-4 space-y-2">
          ...
          {availableConsumables.length > 0 ? (
            availableConsumables.map((item: CharacterConsumable) => (
              <ConsumableItem
