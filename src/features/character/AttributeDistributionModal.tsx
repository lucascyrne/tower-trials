...
interface AttributeDistributionModalProps {
  isOpen: boolean;
  onClose: () => void;
  character: GamePlayer | null; // CORRIGIDO: Character pode ser null durante inicialização
...
const AttributeDistributionModal: React.FC<AttributeDistributionModalProps> = ({
  ...
  // ✅ PROTEÇÃO: Verificar se character existe antes de usar
  ...
  const handleSubmit = async () => {
    if (totalPointsToDistribute === 0 || !character) return;
  ...
  // ✅ PROTEÇÃO: Verificar se character existe e tem pontos antes de renderizar
  if (!isOpen || !character || !character.attribute_points || character.attribute_points === 0)
    return null;
      ...
      >
          ...
          >
            ...
            <div className={isMobileLandscape ? 'text-left flex-1' : 'text-center'}>
              ...
              <DialogDescription asChild>
                <div
                  className={`${isMobileLandscape ? 'flex items-center justify-between mt-1' : 'text-center space-y-3'}`}
                >
                  <Badge
                    variant="outline"
                  ...
                  {!isMobileLandscape && (
                    <p className="text-sm text-muted-foreground">
                      Distribua seus pontos estrategicamente para fortalecer seu personagem na
                      torre.
                    </p>
                  )}
                </div>
        ...
        <div className={`${isMobileLandscape ? 'overflow-y-auto max-h-[calc(95vh-120px)]' : ''}`}>
          ...
          >
            ...
            <div className={`${isMobileLandscape ? 'grid grid-cols-2 gap-2 mb-3' : 'space-y-3'}`}>
                  ...
                  >
                    ...
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        {!isMobileLandscape && (
                          <div className="text-xs text-muted-foreground">{config.benefit}</div>
                        )}
                      </div>
                    </div>
                    ...
                    >
                      <div className={`font-bold ${isMobileLandscape ? 'text-xs' : 'text-sm'}`}>
                        {currentValue}
                        {distribution[attribute] > 0 && (
                          <span className="text-green-400 ml-1">+{distribution[attribute]}</span>
                        )}
                      </div>
                    </div>
                    ...
                    <div className="flex items-center gap-1">
                      <Button
                        size="icon"
                        variant="ghost"
                        className={`rounded-full ${isMobileLandscape ? 'h-6 w-6' : 'h-8 w-8'}`}
            ...
            {totalPointsToDistribute > 0 && (
              ...
              >
                ...
                >
                  <Target className={`text-primary ${isMobileLandscape ? 'h-3 w-3' : 'h-4 w-4'}`} />
                  Preview dos Melhoramentos
                </div>
                <div
        ...
        <div className={`flex gap-2 ${isMobileLandscape ? 'pt-2' : 'pt-2'}`}>
          <Button
            variant="outline"
            onClick={onClose}
            className={`flex-1 ${isMobileLandscape ? 'text-xs h-8' : ''}`}
          >
            Cancelar
          </Button>
          ...
          {totalPointsToDistribute > 0 && (
            <Button
              ...
              className={`flex-1 ${isMobileLandscape ? 'text-xs h-8' : ''}`}
            >
              <RotateCcw className={`mr-1 ${isMobileLandscape ? 'h-3 w-3' : 'h-4 w-4 mr-2'}`} />
          ...
          >
            {isDistributing ? (
              <div
                className={`animate-spin rounded-full border-2 border-white border-t-transparent mr-1 ${
                  isMobileLandscape ? 'h-3 w-3' : 'h-4 w-4 mr-2'
                }`}
              />
            ) : (
              <Save className={`mr-1 ${isMobileLandscape ? 'h-3 w-3' : 'h-4 w-4 mr-2'}`} />
            )}
            {isMobileLandscape
              ? `OK (${totalPointsToDistribute})`
