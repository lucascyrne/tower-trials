      ...
      {isOpen && (
        <Dialog open={isOpen}>
          <DialogContent className={`overflow-hidden ${
            isMobileLandscape 
              ? 'max-w-[95vw] max-h-[90vh] p-3 victory-modal-landscape' 
              : 'sm:max-w-[425px]'
          }`}>
            {/* Header adaptativo */}
              ...
              >
                {isMobileLandscape && (
                  <motion.div
                    ...
                    animate={{ scale: 1 }}
                    transition={{ type: "spring", duration: 0.5 }}
                    className="w-12 h-12 bg-yellow-500/20 rounded-full flex items-center justify-center flex-shrink-0"
                  >
                ...
                <div className={isMobileLandscape ? 'text-left' : 'text-center'}>
                  <DialogTitle className={`font-bold ${isMobileLandscape ? 'text-lg' : 'text-2xl'}`}>
                    Vitória!
                  </DialogTitle>
                  <DialogDescription className={isMobileLandscape ? 'text-sm' : ''}>
            ...
            <div className={isMobileLandscape ? 'scroll-area overflow-y-auto' : ''}>
              ...
              <div className={`${
                ...
                >
                  <Star className={`text-yellow-500 ${isMobileLandscape ? 'h-5 w-5' : 'h-6 w-6'}`} />
                  <div>
                    <div className={`font-semibold ${isMobileLandscape ? 'text-sm' : ''}`}>Experiência</div>
                    <div className={`font-bold text-primary ${isMobileLandscape ? 'text-lg' : 'text-2xl'}`}>
                      +{rewards.xp} XP
                    </div>
                  </div>
                </motion.div>
                ...
                <motion.div 
                  ...
                  whileHover={{ scale: 1.02 }}
                >
                  <Coins className={`text-yellow-400 ${isMobileLandscape ? 'h-5 w-5' : 'h-6 w-6'}`} />
                  <div>
                    <div className={`font-semibold ${isMobileLandscape ? 'text-sm' : ''}`}>Ouro</div>
                    <div className={`font-bold text-primary ${isMobileLandscape ? 'text-lg' : 'text-2xl'}`}>
              ...
              {leveledUp && newLevel && (
                ...
                >
                  <div className={`flex items-center gap-2 ${
                    ...
                    <div className={`font-bold ${isMobileLandscape ? 'text-base' : 'text-xl'}`}>
                      Nível {newLevel}!
                    </div>
                  </div>
              ...
              {rewards.drops.length > 0 && (
                ...
                >
                  <h4 className={`font-semibold flex items-center gap-2 ${
                    isMobileLandscape ? 'mb-1 text-sm' : 'mb-3'
                  }`}>
                    <ShoppingBag className={isMobileLandscape ? 'h-4 w-4' : 'h-5 w-5'} />
                    Itens Obtidos
                  </h4>
                  ...
                  <div className={`${
                    ...
                    {rewards.drops.map((drop, index) => (
                      ...
                      >
                        <span className={isMobileLandscape ? 'text-sm' : 'text-xl'}>🎁</span>
