...
export default function GameBattle() {
  ...
  useEffect(() => {
    ...
    const initializeBattleForCharacter = async () => {
      try {
        ...
        console.log(
          `[GameBattle] Personagem carregado: ${character.name} (andar ${character.floor})`
        );
  ...
  useEffect(() => {
    ...
    const isEnemyTurnStuck =
      isBattleActive && !isPlayerTurn && !loading.performAction && !gameState.battleRewards;
  ...
  useEffect(() => {
    ...
    if (lastBattleStateRef.current.floor !== player.floor) {
      console.log(
        `[GameBattle] Novo andar detectado (${lastBattleStateRef.current.floor} -> ${player.floor}), limpando cache`
      );
    ...
    console.log(
      `[GameBattle] Processando recompensa de vitória: XP ${gameState.battleRewards.xp}, Gold ${gameState.battleRewards.gold}`
    );
  ...
  useEffect(() => {
    ...
    if (isFugaDetected && !showFleeOverlay) {
      ...
      console.log(
        `[GameBattle] ${isSuccess ? '✅ FUGA BEM-SUCEDIDA' : '❌ FUGA FALHOU'} - Preparando overlay`
      );
  ...
  const handleAction = async (action: ActionType, spellId?: string) => {
    ...
    if (currentTime - lastActionTimeRef.current < 300) {
      console.warn(
        `[GameBattle] ⏱️ Debounce - aguardando ${300 - (currentTime - lastActionTimeRef.current)}ms`
      );
      return;
    }
    ...
    if (fleeSuccess) {
      ...
      try {
        ...
        try {
          ...
          console.error(
            '[GameBattle] ⚠️ Erro ao resetar andar (continuando mesmo assim):',
            updateError
          );
        }
      ...
      if (gameState.mode === 'fled') {
        console.warn(
          '[GameBattle] ⚠️ Modo corrigido automaticamente - o sistema deve retornar ao battle mode'
        );
