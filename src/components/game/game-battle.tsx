...
import { Shield, Sword, Star, Sparkles, ShoppingBag, Backpack } from 'lucide-react';
import { Progress } from '@/components/ui/progress';
import { useGame } from '@/resources/game/game-hook';
import { ActionType, BattleActionResult } from '@/resources/game/game-model';
...
export default function GameBattle() {
  ...
  const handleAction = async (action: ActionType, spellId?: string) => {
    ...
    const result = await performAction(action, spellId) as unknown as BattleActionResult;
    
    if (result?.enemyDefeated && result.rewards) {
      await handleEnemyDefeat({
        xp: result.rewards.xp,
        gold: result.rewards.gold,
        drops: result.rewards.drops || [],
        leveledUp: result.rewards.leveledUp,
        newLevel: result.rewards.newLevel
      });
    }
  };

  const handleEnemyDefeat = async (rewards: BattleRewards) => {
    setVictoryRewards({
      xp: rewards.xp,
      gold: rewards.gold,
      drops: rewards.drops || [],
