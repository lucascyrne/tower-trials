...
export const FleeOverlay: React.FC<FleeOverlayProps> = ({
  ...
  useEffect(() => {
    ...
    console.log(`[FleeOverlay] 🎭 Overlay ativado - Sucesso: ${isSuccess}, Jogador: ${playerName}, Inimigo: ${enemyName}`);
  ...
  }, [isVisible, isSuccess, playerName, enemyName]);
  ...
  const getMessage = () => {
    if (isSuccess) {
      return {
        ...
        description: enemyName ? `${playerName} conseguiu fugir de ${enemyName}!` : `${playerName} conseguiu escapar!`,
      ...
      return {
        ...
        description: enemyName ? `${enemyName} impediu a fuga de ${playerName}!` : `O inimigo impediu a fuga de ${playerName}!`,
  ...
  return (
    <div className="fixed inset-0 backdrop-blur-md z-50 flex items-center justify-center">
      ...
      <div className={`relative ${message.bgColor} border ${message.borderColor} rounded-2xl p-8 shadow-2xl max-w-md mx-4 transform transition-all duration-500 animate-in zoom-in-95 fade-in-0`}>
        ...
        <div className="text-center space-y-4">
          ...
          <div className={`font-bold text-2xl ${message.textColor}`}>
            {message.title}
          </div>
          ...
          <div className="flex items-center justify-center gap-3 pt-2">
            <div className={`animate-spin rounded-full h-5 w-5 border-2 border-t-transparent ${message.accentColor}`}></div>
        ...
        <div className={`absolute inset-0 rounded-2xl border-2 ${message.borderColor} animate-pulse pointer-events-none`}></div>
      </div>
    </div>
  );
}; 