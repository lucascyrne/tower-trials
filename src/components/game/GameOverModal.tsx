...
export function GameOverModal({
  ...
  const handleFinalizeGameOver = async () => {
    ...
    try {
      ...
      const rankingResponse = await RankingService.saveScore({
        ...
        highest_floor: player.floor - 1, // -1 porque morreu no andar atual
        user_id: userId,
  ...
  const handleViewRanking = () => {
    window.location.href = '/game/ranking';
  };
    ...
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className={`overflow-hidden ${
        ...
        <DialogHeader className={`text-center ${isMobileLandscape ? 'pb-2' : ''}`}>
          <DialogTitle className={`flex items-center justify-center gap-2 ${
            ...
            <Skull className={`text-red-500 ${
              isMobileLandscape ? 'h-5 w-5' : 'h-8 w-8'
            }`} />
          ...
          <div className="text-center">
            ...
            <div className={`flex items-center justify-center gap-2 mb-2 ${
              ...
              <Trophy className={`text-yellow-500 ${
                isMobileLandscape ? 'h-4 w-4' : 'h-6 w-6'
              }`} />
            ...
            <div className={`bg-red-500/10 border border-red-500/20 rounded-lg ${
              ...
              <div className={`flex items-center gap-2 mb-2 ${
                ...
                <AlertTriangle className={`text-red-400 ${
                  isMobileLandscape ? 'h-3 w-3' : 'h-5 w-5'
                }`} />
        ...
        <div className={`flex gap-2 ${
          isMobileLandscape ? 'flex-row pt-2' : 'flex-col pt-4'
        }`}>
          {isCharacterDeleted && (
            <Button
              variant="outline"
          ...
          >
            {isSaving || isDeleting ? (
              ...
              <span className="flex items-center gap-1">
                ...
                {isMobileLandscape 
                  ? 'Aceitar Derrota' 
                  : (isCharacterDeleted ? 'Criar Novo Personagem' : 'Aceitar Derrota e Salvar')
                }
              </span>
            )}
          </Button>
