...
export const Route = createFileRoute('/_authenticated/game/play/equipment/select')({
  ...
  const handleEquipItem = async (equipment: Equipment) => {
    ...
    try {
      ...
      if (result.success) {
        ...
        navigate({ to: '/game/play/equipment', search: { character: characterId } })
      } else {
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          <div className="flex flex-col sm:flex-row sm:items-center gap-4">
            <Button 
              variant="outline" 
              size="sm"
              onClick={() => navigate({ to: '/game/play/equipment', search: { character: characterId } })}
