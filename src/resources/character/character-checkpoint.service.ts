...
import { CharacterHealingService } from '@/resources/character/character-healing.service';
...
export class CharacterCheckpointService {
  ...
  static async getUnlockedCheckpoints(characterId: string): Promise<ServiceResponse<Checkpoint[]>> {
    try {
      ...
      // ✅ CORREÇÃO: Usar apenas character.floor como o andar mais alto alcançado
      // O campo 'floor' na tabela characters representa o andar atual/máximo do personagem
      const highestFloor = Math.max(1, character.floor || 1);

      console.log(`[CharacterCheckpointService] Gerando checkpoints para andar ${highestFloor}`);
      ...
      console.log(`[CharacterCheckpointService] Checkpoints gerados:`, checkpoints);
