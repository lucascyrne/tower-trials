...
export class BattleService {
  ...
  static calculateDamage(
    ...
    const enhancedDoubleAttackChance = doubleAttackChance + Math.floor((attackerDexterity - 10) * 0.5) + Math.floor((attackerSpeed - 10) * 0.3);
            ...
            if (attackSkillXp.length > 0) {
              ...
              try {
                ...
                for (const levelUp of skillLevelUps) {
                  console.log(`[BattleService] Skill level up: ${levelUp.skill} -> ${levelUp.newLevel}`);
                }
              } catch (error) {
  ...
  static async processFleeAttempt(gameState: GameState): Promise<GameState> {
    ...
    if (fleeSuccess) {
      ...
      if (newHp <= 0) {
        return this.processPlayerDeath(gameState, player, currentEnemy, `${player.name} tentou fugir mas falhou!`);
      }
  ...
  static async processEnemyAction(
    ...
    console.log(`[BattleService] Ação escolhida: ${actionType} (special: ${specialChance}, spell: ${spellChance})`);
    ...
    switch (actionType) {
        ...
        if (newHp <= 0) {
          console.log(`[BattleService] Jogador ${player.name} morreu - iniciando processo de permadeath`);
          return {
      ...
      case 'spell': {
        ...
        if (newSpellHp <= 0) {
          console.log(`[BattleService] Jogador ${player.name} morreu por magia - iniciando processo de permadeath`);
          return {
            newState: await this.processPlayerDeath(gameState, player, enemy, message)
          };
        }
      ...
      case 'special': {
        ...
        if (specialAbilities.length > 0) {
          ...
          if (randomAbility.includes('Regenera') || randomAbility.includes('Recupera') || randomAbility.includes('cura')) {
            const healAmount = Math.floor(enemy.maxHp * (0.10 + Math.random() * 0.15));
        ...
        if (newSpecialHp <= 0) {
          console.log(`[BattleService] Jogador ${player.name} morreu por habilidade especial - iniciando processo de permadeath`);
          return {
            newState: await this.processPlayerDeath(gameState, player, enemy, message)
          };
        }
        ...
        console.log(`[BattleService] Habilidade especial processada com sucesso. Mensagem: ${message}`);
