  ...
  static async processEnemyDefeat(gameState: GameState): Promise<GameState> {
    try {
      ...
      if (gameState.battleRewards) {
        console.warn('[RewardService] Tentativa de processar derrota de inimigo que já possui recompensas - ignorando');
        return gameState;
      }
      ...
      console.log(`[RewardService] XP persistido - Level: ${xpData.new_level}, XP: ${xpData.new_xp}, Level Up: ${xpData.leveled_up}`);
      ...
      if (currentEnemy.possible_drops && currentEnemy.possible_drops.length > 0) {
        console.log(`[RewardService] Monstro ${currentEnemy.name} tem ${currentEnemy.possible_drops.length} possible_drops`);
        ...
        if (dropsObtidos.length > 0) {
          ...
          if (dropInfoResponse.success && dropInfoResponse.data) {
            ...
            console.log(`[RewardService] Drops identificados:`, drops.map(d => `${d.quantity}x ${d.name}`).join(', '));
          } else {
            console.error(`[RewardService] Erro ao buscar informações dos drops:`, dropInfoResponse.error);
          ...
          console.log(`[RewardService] ${addDropsResult.data} drops persistidos com sucesso no inventário`);
        }
      }
      ...
      console.log(`[RewardService] - Level: ${updatedPlayer.level} (Level Up: ${xpData.leveled_up})`);
