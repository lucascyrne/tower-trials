...
export class CharacterService {
  ...
  private static pendingUserCharactersRequests: Map<string, Promise<ServiceResponse<Character[]>>> =
    new Map();
    ...
    try {
      ...
      if (now - lastFetch < 10000) {
        ...
        if (cachedCharacters.length > 0) {
          console.log(
            `[CharacterService] Usando lista de personagens em cache para usuário ${userId} (${cachedCharacters.length} personagens) - última busca há ${Math.round((now - lastFetch) / 1000)}s`
          );
  ...
  static async getCharacter(characterId: string): Promise<ServiceResponse<Character>> {
    try {
      ...
      if (cachedCharacter && now - lastFetch < this.CACHE_DURATION) {
        console.log(
          `[CharacterService] Usando personagem em cache: ${cachedCharacter.name} (andar: ${cachedCharacter.floor})`
        );
      ...
      if (result.success && result.data) {
        ...
        if (healResult.success && healResult.data && healResult.data.healed) {
          ...
          if (healPercent >= 5) {
            ...
            console.log(
              `[CharacterService] Cura significativa detectada: +${healAmount} HP (${healPercent.toFixed(1)}%)`
            );
          }
   ...
   * @private
   */
  private static async fetchUserCharactersFromServer(
    userId: string,
    userCacheKey: string,
    now: number
    ...
    try {
      ...
      console.error(
        'Erro ao buscar personagens do servidor:',
        error instanceof Error ? error.message : error
      );
      return {
        data: null,
  ...
  private static async fetchCharacterFromServer(
    ...
    try {
      ...
      console.log(
        `[CharacterService] Personagem carregado do servidor: ${character.name} (andar: ${character.floor}) com crítico: ${character.critical_chance}`
      );
  ...
  static async updateCharacterFloor(
    ...
    try {
      console.log(
        `[CharacterService] Atualizando andar seguramente - ID: ${characterId}, Andar: ${newFloor}`
      );
  ...
  static invalidateCharacterCache(characterId: string): void {
    ...
    this.cacheInvalidationTimer = setTimeout(() => {
      ...
      if (idsToInvalidate.length > 0) {
        console.log(
          `[CharacterService] Cache invalidado para ${idsToInvalidate.length} personagens: ${idsToInvalidate.join(', ')}`
        );
      }
  ...
  static invalidateUserCache(userId: string): void {
    ...
    console.log(
      `[CharacterService] Cache de usuário ${userId} invalidado (${userCharacterIds.length} personagens)`
    );
  }
   ...
   */
  static async getUnlockedCheckpoints(
    characterId: string
  ): Promise<ServiceResponse<{ floor: number; description: string }[]>> {
    try {
      ...
      console.log(
        `[CharacterService] Andar atual: ${character.floor}, Highest floor: ${highestFloor}`
      );
      ...
      for (let i = 1; i <= 100; i++) {
        ...
        if (highestFloor >= checkpointFloor) {
          ...
          console.log(
            `[CharacterService] Checkpoint ${checkpointFloor} desbloqueado (passou do boss ${bossFloor})`
          );
        } else {
  ...
  static async applyAutoHeal(
    ...
    ServiceResponse<{ healed: boolean; oldHp: number; newHp: number; character: Character }>
  > {
    try {
      const characterResponse = await this.getCharacter(characterId);
      if (!characterResponse.success || !characterResponse.data) {
  ...
  private static async calculateDerivedStatsFallback(character: Character): Promise<{
    ...
    console.log(
      `[CharacterService] Build analysis - Diversidade: ${(attributeDiversity * 100).toFixed(1)}%, Mono-penalty: ${monoPenalty}`
    );
