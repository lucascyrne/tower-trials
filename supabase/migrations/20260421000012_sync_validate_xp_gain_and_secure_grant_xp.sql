-- Sincroniza validação anti-cheat de XP e concessão segura para evitar drift.
-- Mantém proteção contra abuso, mas sem bloquear recompensas legítimas de combate.
...
BEGIN
    ...
    CASE p_source
        WHEN 'combat' THEN
            -- Combate (incluindo boss) precisa de margem maior.
            v_max_xp_per_source := GREATEST(300, v_character_level * 25);
        WHEN 'quest' THEN
...
BEGIN
    ...
    IF v_leveled_up THEN
        SELECT hp, max_hp, mana, max_mana, atk, def, speed
        INTO v_base_stats
    ...
    ELSE
        UPDATE characters
        SET xp = v_new_xp, last_activity = NOW()
        WHERE id = p_character_id;
    END IF;
