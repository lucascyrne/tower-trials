-- Migração: Corrigir Tipo de Retorno da Coluna Name
-- Data: 2024-12-21
-- Versão: 20241221000005
-- Objetivo: Corrigir erro "Returned type text does not match expected type character varying in column 2"
...
CAUSA: A coluna name na função get_monster_for_floor_with_initiative está declarada como VARCHAR
       mas o resultado da concatenação de strings retorna TEXT, causando incompatibilidade
*/

-- =====================================
-- SOLUÇÃO: ALTERAR TIPO DE RETORNO PARA TEXT
-- =====================================

-- Remover função com tipo incorreto
DROP FUNCTION IF EXISTS get_monster_for_floor_with_initiative(INTEGER) CASCADE;

-- Recriar função com tipo correto para name (TEXT ao invés de VARCHAR)
CREATE OR REPLACE FUNCTION get_monster_for_floor_with_initiative(p_floor INTEGER)
RETURNS TABLE(
    id TEXT,
    name TEXT,                  -- CORRIGIDO: VARCHAR → TEXT
    level INTEGER,
    hp INTEGER,
    atk INTEGER,
    def INTEGER,
...
BEGIN
    ...
    -- RETORNO CORRIGIDO: UUID→TEXT e concatenação de strings produz TEXT
    RETURN QUERY SELECT
        v_monster.id::TEXT,                               -- UUID → TEXT
        (v_monster.name || CASE 
            WHEN v_selected_build != 'balanced' THEN ' ' || initcap(v_selected_build)
            ELSE ''
        END)::TEXT,                                       -- CONVERSÃO EXPLÍCITA PARA TEXT
        ...
        v_stats.speed,
        v_monster.behavior::VARCHAR,                      -- ENUM → VARCHAR
        v_monster.min_floor,
...
'Versão corrigida que usa TEXT para name para resolver incompatibilidade de tipos';
